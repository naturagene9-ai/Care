/* =====================================================
   NaturaGene Care v4 — script.js
   Fixed Mobile Nav · Cursor · Particles · Reveal
   Countdown · FAQ · Form + Google Sheets · Razorpay
   ===================================================== */
'use strict';
...
const GA = (ev, params = {}) => { if (typeof gtag !== 'undefined') gtag('event', ev, params); };
...
/* ── MOBILE NAV (fixed: overlay + drawer + lock scroll) ── */
const burger    = $('burger');
...
const sectionObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      ...
      navLinks.forEach(l => { l.style.color = l.getAttribute('href') === `#${id}` ? 'var(--sage-d)' : ''; });
    }
  });
...
const wlForm = $('wlForm');
if (wlForm) {
  ['wl-name','wl-email'].forEach(id => {
    const el = $(id);
    if (el) el.addEventListener('input', () => {
      el.classList.remove('error');
      const errId = { 'wl-name':'en','wl-email':'ee' }[id];
      const errEl = $(errId);
  ...
  wlForm.addEventListener('submit', async e => {
    ...
    $('wl-name')?.classList.remove('error');
    $('wl-email')?.classList.remove('error');
    if ($('en')) $('en').textContent = '';
    if ($('ee')) $('ee').textContent = '';

    if (name.length < 2) {
      $('wl-name')?.classList.add('error');
      if ($('en')) $('en').textContent = 'Please enter your full name.';
      valid = false;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    const payload = {
      type: 'waitlist', name, email, city,
      source: 'landing_page', timestamp: new Date().toISOString(),
...
function openRazorpay() {
  ...
  if (CONFIG.RAZORPAY_KEY.includes('REPLACE')) { showRzpGuide(); return; }

  const options = {
    key: CONFIG.RAZORPAY_KEY, amount: 29900, currency: 'INR',
    name: 'NaturaGene Care',
    description: 'Sunflower Soap with Vitamin C — Early Bird Pre-Order',
    prefill: { name, email },
    notes: { product: 'sunflower-soap-vc', source: 'landing_page_v4' },
    theme: { color: '#7AA874' },
...
async function onPaySuccess(response) {
  const payload = {
    type: 'preorder', payment_id: response.razorpay_payment_id,
    order_id: response.razorpay_order_id || '', amount: 299, currency: 'INR',
    product: 'sunflower-soap-vc', timestamp: new Date().toISOString(),
  };
  ...
  GA('purchase', {
    transaction_id: response.razorpay_payment_id, value: 299, currency: 'INR',
...
function showRzpGuide() {
  alert(
    ...
    '3. Open script.js → replace RAZORPAY_KEY in CONFIG\n\n' +
...
$$('.fq').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    $$('.fq.open').forEach(b => { b.classList.remove('open'); b.nextElementSibling?.classList.remove('open'); });
    if (!isOpen) {
...
const secTrackObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) GA('section_view', { event_category: 'scroll_depth', event_label: e.target.id || e.target.className.split(' ')[0] });
  });
}, { threshold: 0.35 });
$$('section[id]').forEach(s => secTrackObs.observe(s));

/* ── PARALLAX HERO CARD (desktop) ───────────────────── */
...
(function () {
  const statEl = $('statWaitlist');
  if (!statEl) return;
  const target = 500 + Math.floor(Math.random() * 80);
  let started = false;
...
console.log(
  '%cNaturaGene Care 🌻 v4.0\n%c\n' +
  '✅ Setup: Razorpay Key, Sheets URL, GA ID, GSC code, Launch Date\n' +
  'Local data: localStorage.getItem("ng_waitlist") / "ng_orders"',
  'color:#7AA874;font-size:14px;font-weight:bold', 'color:#555;font-size:11px'
);
